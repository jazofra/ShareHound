...
Derived edges that represent what a principal can **actually** do when accessing a file or
directory over SMB.  They are emitted only when the same SID holds matching generic rights
at **both** the share level and the NTFS level — because Windows enforces both DACLs and
grants only the intersection.
...
> through a group SID but holds NTFS read under their personal SID (or vice versa),
> no effective edge is emitted.  BloodHound's AD graph can close this gap via group
> membership traversal at query time.
